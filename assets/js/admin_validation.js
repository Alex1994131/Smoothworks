...
$(document).ready(function () {
    ...
    $('#membership_form').bootstrapValidator({
        // To use feedback icons, ensure that you use Bootstrap v3.1.0 or later
        fields: {
            membership_title: {
                validators: {
                    notEmpty: {
                        message: 'Please enter the membership title.'
                    }
                }
            },
            membership_kind: {
                validators: {
                    notEmpty: {
                        message: 'Please enter the membership kind.'
                    }
                }
            },
            membership_level: {
            ...
            memberships: {
                validators: {
                    notEmpty: {
                        message: 'Please enter the memberships.'
                    }
                }
            },
        }
    ...
    $('#admin_add_membership_detail').bootstrapValidator({
        // To use feedback icons, ensure that you use Bootstrap v3.1.0 or later
        fields: {
            membership_detail_title: {
                validators: {
                    notEmpty: {
                        message: 'Please enter a Membership title'
                    }
                }
            },
            membership_detail_key: {
                validators: {
                    notEmpty: {
                        message: 'Please enter the membership key.'
                    }
                }
            },
        }
    ...
    $('#admin_add_membership_group').bootstrapValidator({
        // To use feedback icons, ensure that you use Bootstrap v3.1.0 or later
        fields: {
            membership_group: {
                validators: {
                    notEmpty: {
                        message: 'Please enter a Membership Group'
                    },
                    remote: {
                        url: BASE_URL + 'admin/membership/membershipgroupcheck',
                        data: function (validator) {
                            return {
                                membership_group: validator.getFieldElements('membership_group').val()
                            };
                        },
                        message: 'The Membership group is already there !!!',
                        type: 'POST'
                    }
                }
            }
...
function delete_membership(val) {
    bootbox.confirm("Are you sure want to Delete ? ", function (result) {
        //alert(result)
        if (result == true) {
            var url = BASE_URL + 'admin/membership/delete_membership';
            var tbl_id = val;
            $.ajax({
                url: url,
                data: {tbl_id: tbl_id},
                ...
                success: function (res) {
                    if (res == 1) {
                        window.location = BASE_URL + 'admin/membership';
                    }
                }
            });
        }
...
function delete_membership_detail(val) {
    bootbox.confirm("Are you sure want to Delete ? ", function (result) {
        //alert(result)
        if (result == true) {
            var url = BASE_URL + 'admin/membership/delete_membership_detail';
            var tbl_id = val;
            $.ajax({
                url: url,
                data: {tbl_id: tbl_id},
                ...
                success: function (res) {
                    if (res == 1) {
                        window.location = BASE_URL + 'admin/membership_detail';
                    }
                }
            });
        }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.total_count > 0) {
                $("#notification_count").html(data.total_count);
