...
$(document).ready(function () {
    ...
    $('#common_search').bind('keypress', function (event) {
        ...
        var url = base_url + 'user/gigs/common_search/' + common_selected_category;
    ...
    $('#forget_form').bootstrapValidator({
        fields: {
            forget_email: {
                validators: {
                    remote: {
                        url: base_url + 'welcome/check_registered_email',
                        data: function (validator) {
                            return {
        ...
        var url = base_url + 'welcome/forgot_password';
    ...
    $('#users_register').bootstrapValidator({
        fields: {
            ...
            username: {
                validators: {
                    remote: {
                        url: base_url + 'welcome/check_username',
                        data: function (validator) {
                            return {
            ...
            email: {
                validators: {
                    remote: {
                        url: base_url + 'welcome/check_available_email',
                        data: function (validator) {
                            return {
        ...
        $.ajax({
            type: 'POST',
            url: base_url + 'welcome/users_registeration',
            data: {
                username: username,
                password: password,
                email: email,
    ...
    $('#affiliate_users_register').bootstrapValidator({
        fields: {
            name: {
                validators: {
                    notEmpty: {
        ...
        $('#affiliate_users_register').data('bootstrapValidator').resetForm();
        
        var username = $('#affiliate_username').val();
        var password = $('#affiliate_repeatpassword').val();
        var email = $('#affiliate_email').val();
        var name = $('#affiliate_name').val();
        var country_id = $('#affiliate_country_id').val();
        var state_id = $('#affiliate_state_id').val();
        var account_type_id = $("#affiliate_account_type_id").val();
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response == 1) {
                    $('#affiliate_register_success').html('<div class="account-error success">' + thanks + ' ! ' + activation_mail_has_been_sent_to_registered_mail_id + '</div>');
                    window.setTimeout(function () {
                        window.location.href = base_url;
                    }, 100);
                } else if (response == 2) {
                    $('#affiliate_register_success').html('<div class="account-error">' + something_went_wrong + '</div>');
                }
            }
        });
    ...
    $('#users_login').bootstrapValidator({
        ...
        $.ajax({
            type: 'POST',
            url: base_url + 'welcome/is_valid_login',
            data: {
                username: username,
                password: password
            },
            success: function (response) {
                if (response == 1) {
                    ...
                    document.location.href = base_url + 'user/membership';
                }
            }
        });
    });
...
function check_extra_gigs() {
    ...
    var url = base_url + 'user/gigs/extra_gig_calculations';
    $.ajax({
        type: 'post',
        url: url,
        data: {
...
function remove_files(filename, row_id, file_type) {
    ...
    var url = base_url + 'user/gigs/delete_uploaded_file';
    ...
    $('#third_party_videos').click(function () {
        ...
        if ((youtube_url.length > 0) || (vimeo_url.length > 0)) {
            ...
            if (youtube_url.length > 0) {
                ...
                var url = base_url + 'user/gigs/you_tube_links';
            ...
            if (vimeo_url.length > 0) {
                ...
                var url = base_url + 'user/gigs/vimeo_links';
            ...
            if ($.inArray(ext, ['gif', 'png', 'jpg', 'jpeg']) == -1) {
                ...
                img.onload = function () {
                    ...
                    if (img.width >= 680 && img.height >= 460) {
                        ...
                        var url = base_url + 'user/gigs/file_upload';
...
$(function () {
    ...
    var url = base_url + 'user/gigs/get_country_list';

    var submit_url = base_url + 'user/gigs/search';
...
$(function () {
    ...
    var url = base_url + 'user/gigs/all_categories';
...
$('#affiliate_country_id').change(function () {
    var country_id = $(this).find('option:selected').val();
    var url = base_url + 'get_state_list';
    if (country_id != '') {
        $.ajax({
...
$(document).ready(function () {
    $('#save').click(function () {
        ...
        var ajax_update = '';
        var url = base_url + 'user/gigs/update_name';
        $.ajax({
            type: 'POST',
            url: url,
            data: {
    ...
    $('#save_language').click(function () {
        ...
        var url = base_url + 'user/gigs/update_language';
        $.ajax({
            type: 'POST',
            url: url,
            data: {
