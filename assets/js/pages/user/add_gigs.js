...
function gig_title_check(e) {
    var gig_title = $(e).val();
    $.post(base_url + 'user/gigs/check_gig_title', {gig_title: gig_title}, function (message) {
    ...
    $('.extra_money_price').each(function () {
        ...
        if (no != '#' && typeof no !== "undefined") {
            if ($('#label_name_' + no).val() != "" || $('#label_val_' + no).val() != "") {
                if ($('#label_name_' + no).val() == "") {
                    $('.extra_gigs_gig_name').show();
                    error = 1;
                    return false;
                } else {
                    $('.extra_gigs_gig_name').hide();
                }
                if ($('#label_val_' + no).val() == "") {
                    $('.extra_gigs_validate').show();
                    error = 1;
                    return false;
                } else {
                    $('.extra_gigs_validate').hide();
                }
            }
        }
    });
