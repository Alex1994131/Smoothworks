...
class Search extends CI_Controller
{
    ...
    public function index($start = 0)
    {
        if ($this->input->get('search_value')) {
            ...
            $query = $this->db->query("
                SELECT * FROM sell_gigs S LEFT JOIN members M ON M.USERID = S.user_id  WHERE S.status = 0  AND M.status = 0  " . $append_sql);
            ...
            if (!empty($this->uri->segment('3'))) {
                $category_values = str_replace('-', ' ', $this->uri->segment('3'));
    ...
    public function category()
    {
        ...
        if (isset($_GET['search_value']) && !empty($_GET['search_value'])) {
            $search_value = str_replace("|", '&', rawurldecode(utf8_decode($_GET['search_value'])));
            $append_suffix = "/?search_value=" . str_replace('&', '|', $search_value);
        } else {
