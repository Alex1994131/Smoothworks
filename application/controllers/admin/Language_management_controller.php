...
class Language_management_controller extends CI_Controller
{
...

    {
        ...
        if ($this->input->post()) {
            if ($this->data['admin_id'] > 1) {
                ...
                $result = $this->language->get_keywords();

                if ($result) {

                    $this->session->set_flashdata('message', 'The Other Language Keyword has been added successfully...');
...

    {
        ...
        redirect(base_url('admin/language_management_controller/other_language_keys/' . $current_page));
