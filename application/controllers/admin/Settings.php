...
class Settings extends CI_Controller
{
    ...
    public function index()
    {
            ...
            if ($this->data['admin_id'] > 1) {
                ...
                if ($_FILES['site_banner']['name']) {
                    ...
                    if ($this->upload->do_upload('site_banner')) {
                        $img_uploadurl = 'uploads/logo' . $_FILES['site_banner']['name'];
                        $key = 'logo_banner';
                        $val = 'uploads/logo/' . $image_name;
                        $this->db->where('key', $key);
                    }
                    $this->db->delete('system_settings');
                    $website_banner['key'] = $key;
                    $website_banner['value'] = $val;
                    $website_banner['system'] = 1;
                    $website_banner['groups'] = 'config';
                    $website_banner['update_date'] = date('Y-m-d');
                    $website_banner['status'] = 1;
                    $insert = $this->db->insert('system_settings', $website_banner);
                }
                //website banner end
                ...
                if ($data) {
                    ...
                    foreach ($data as $key => $val) {
                        ...
                        if ($key != 'form_submit' && $key != 'social_facebook' && $key != 'social_twitter' && $key != 'social_google' && $key != 'social_linkedin' && $key != 'social_instagram' && $key != 'social_android' && $key != 'social_ios') {
                            ...
                            if($key == 'facebook')
                            {
                                $table_data['visible'] = $this->input->post('social_facebook');
                            }
                            else if($key == 'twitter'){
                                $table_data['visible'] = $this->input->post('social_twitter');
                            }
                            else if($key == 'google_plus'){
                                $table_data['visible'] = $this->input->post('social_google');
                            }
                            else if($key == 'linkedIn'){
                                $table_data['visible'] = $this->input->post('social_linkedin');
                            }
                            else if($key == 'instagram'){
                                $table_data['visible'] = $this->input->post('social_instagram');
                            }
                            else if($key == 'android'){
                                $table_data['visible'] = $this->input->post('social_android');
                            }
                            else if($key == 'ios'){
                                $table_data['visible'] = $this->input->post('social_ios');
                            }
                            else{
                                $table_data['visible'] = '';
                            }
        ...
        foreach ($results as $config) {
            if($config['key'] == 'facebook' || $config['key'] == 'twitter' || $config['key'] == 'google_plus' || $config['key'] == 'linkedIn' || $config['key'] == 'instagram' || $config['key'] == 'android' || $config['key'] == 'ios') {
