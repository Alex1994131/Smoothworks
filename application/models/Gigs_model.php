...
class Gigs_model extends CI_Model
{
    ...
    {
        $query = $this->db->query("
            SELECT payments.price, sell_gigs.title, payments.currency, members.email as seller_email , (members.fullname) as seller_name , (members.username) as seller_username, gigs_image.gig_image_thumb,
            (SELECT (members.username)  FROM members WHERE USERID =  payments.`user_id` ) as buyer_username,
            (SELECT (members.email)  FROM members WHERE USERID =  payments.`user_id` ) as buyer_email 
            FROM `payments` 
            LEFT JOIN sell_gigs ON sell_gigs.id = payments.`item_id`
            LEFT JOIN gigs_image ON gigs_image.gig_id = payments.`item_id`
    ...
    public function latest_gigs()
    {
        $query = $this->db->query(
            "SELECT 
                ...
                states.`state_name`,
                (SELECT gigs_image.`image_path` FROM `gigs_image` WHERE gigs_image.gig_id = sell_gigs.idLIMIT 0 , 1  ) AS gig_image,
    ...
    public function popular_gigs($param = '')
    {
        ...
        $query = $this->db->query(
            "SELECT 
                ...
                members.`user_profile_image` , 
                states.`state_name` , 
                (SELECT gigs_image.`gig_image_medium` FROM `gigs_image` WHERE gigs_image.gig_id = sell_gigs.id LIMIT 0 , 1  ) AS gig_image , country.country , members.`state`,
                (SELECT count(id) FROM `feedback` WHERE `gig_id` = sell_gigs.id and to_user_id = sell_gigs.user_id) AS gig_usercount,
                (SELECT AVG(rating) FROM `feedback` WHERE `gig_id` = sell_gigs.id and to_user_id = sell_gigs.user_id) AS gig_rating 
                FROM `sell_gigs` 
                LEFT JOIN members ON members.`USERID` = sell_gigs.user_id
    ...
    public function favourites($user_id, $return_type, $start, $end)
    {
        $append_sql = "";
        if ($start > 0 || $end > 0) {
    ...
    public function location_base_gigs($full_country_name, $param, $start, $end)
    {
        ...
        $query = $this->db->query("
                    ...
                    country.country , 
                    members.`state`,
                    (SELECT gigs_image.`image_path` FROM `gigs_image` WHERE gigs_image.gig_id = sell_gigs.id LIMIT 0 , 1  ) AS gig_image , 
					(SELECT count(id) FROM `feedback` WHERE `gig_id` = sell_gigs.id and to_user_id = sell_gigs.user_id) AS gig_usercount,
					(SELECT AVG(rating) FROM `feedback` WHERE `gig_id` = sell_gigs.id and to_user_id = sell_gigs.user_id) AS gig_rating FROM `sell_gigs` 
                    LEFT JOIN members ON members.`USERID` = sell_gigs.user_id
                    LEFT JOIN country ON members.`country` = country.id
    ...
    public function username_base_gigs($username, $param, $start, $end)
    {
        ...
        $query = $this->db->query("
                    ...
                    LEFT JOIN country ON members.`country` = country.id
                    LEFT JOIN states ON `states`.`state_id` = `members`.`state`
                    WHERE members.`username` = '$username' and sell_gigs.exist_flag = 0
					AND sell_gigs.status = 0
                    ORDER BY sell_gigs.id ASC" . $append_sql . " ;");
    ...
    public function get_gig_details($title)
    {
        ...
        $query = $this->db->query("SELECT  
                    ...
                    categories.name,
                    categories.parent , 
                    GROUP_CONCAT(gigs_image.image_path SEPARATOR '#') as image_path ,
    ...
    public function user_all_gigs($gig_id, $user_id)
    {
        $query = $this->db->query("
                    ...
                    `states`.`state_name` ,
                    (SELECT gigs_image.`gig_image_medium` FROM `gigs_image` WHERE gigs_image.gig_id = sell_gigs.id LIMIT 0 , 1  ) AS gig_image , 
                    (SELECT gigs_image.`gig_image_tile` FROM `gigs_image` WHERE gigs_image.gig_id = sell_gigs.id LIMIT 0 , 1  ) AS gig_image_tile , country.country , members.`state`,
					(SELECT count(id) FROM `feedback` WHERE `gig_id` = sell_gigs.id and to_user_id = sell_gigs.user_id) AS gig_usercount,
					(SELECT AVG(rating) FROM `feedback` WHERE `gig_id` = sell_gigs.id and to_user_id = sell_gigs.user_id) AS gig_rating FROM `sell_gigs` 
                    LEFT JOIN members ON members.`USERID` = sell_gigs.user_id
                    LEFT JOIN country ON members.`country` = country.id
                    LEFT JOIN states ON `states`.`state_id` = `members`.`state`
                    WHERE 
                        members.USERID = '". $user_id ."' AND 
                        sell_gigs.id != '". $gig_id ."' AND 
                        sell_gigs.user_id NOT IN (select USERID from members where  status = 1)  AND 
                        sell_gigs.status = 0
    ...
    public function category_based_gigs($cat_id, $title = '')
    {
        ...
        $query = $this->db->query("
                    ...
                    (SELECT gigs_image.`gig_image_tile` FROM `gigs_image` WHERE gigs_image.gig_id = sell_gigs.id LIMIT 0 , 1  ) AS gig_image_tile , 
                    country.country , members.`state`,
					(SELECT count(id) FROM `feedback` WHERE `gig_id` = sell_gigs.id and to_user_id = sell_gigs.user_id) AS gig_usercount,
					(SELECT AVG(rating) FROM `feedback` WHERE `gig_id` = sell_gigs.id and to_user_id = sell_gigs.user_id) AS gig_rating FROM `sell_gigs` 
                    LEFT JOIN members ON members.`USERID` = sell_gigs.user_id
    ...
    public function similar_gigs($cat_id, $title = '')
    {
        $append_sql = "";
        if (!empty($title)) {
            $append_sql = " AND sell_gigs.`title` != '$title'";
        }
        $query = $this->db->query("
                    SELECT 
                    sell_gigs.*,
    ...
    public function my_gigs($return_type, $user_id, $start, $end)
    {
        ...
        $query = $this->db->query("
                    ...
                    LEFT JOIN country ON members.`country` = country.id
                    LEFT JOIN states ON `states`.`state_id` = `members`.`state`
                    WHERE members.USERID = $user_id and sell_gigs.exist_flag =0
					AND sell_gigs.status = 0
                    ORDER BY sell_gigs.id DESC " . $last_append . " ");
        if ($return_type == 0) {
            $result = $query->num_rows();
