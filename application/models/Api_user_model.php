...
class Api_user_model extends CI_Model
{
    ...
    public function change_account($account_state) {
    ...
    {
        if (!empty($id)) {
            $query = $this->db->get_where('members', array('USERID' => $id));
            return $query->row_array();
        } else {
    ...
    public function get_my_usd_balances($user_id) {
        $query = $this->db->query("select amount from balances where user_id = $user_id and currency_type = 'USD'");
        $result = $query->result_array();
        return $result[0]['amount'];
    }

    public function get_my_currency_balances($user_id, $currency_type) {
        $query = $this->db->query("select amount from balances where user_id = $user_id and currency_type = $currency_type");
        $result = $query->result_array();
        return $result[0]['amount'];
    }
}
